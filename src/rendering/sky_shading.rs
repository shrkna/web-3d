...
use crate::rendering::webgpu::{
    WebGPUInterface, WebGPUUniqueResources,
};
use crate::Shared;
...
pub fn sky_pass(
	...
	if is_first_update {
        global_resources.sky_shading_resource =
            Some(create_sky_shader_resource(&interface));

	// Convert hdr to cube pass
	{
		let mut convert_pass = command_encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
	...
	update_sky_shader_resource(&interface, &scene, global_resources.sky_shading_resource.as_ref().unwrap());
	...
	// Sky render pass
	{
		let mut sky_render_pass = command_encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
			...
			color_attachments: &[Some(wgpu::RenderPassColorAttachment {
				view: &interface.intermediate_texture_2.create_view(&wgpu::TextureViewDescriptor::default()),
				resolve_target: None,
				ops: wgpu::Operations {
...
fn create_sky_shader_resource(interface: &WebGPUInterface) -> WebGPUSkyShadingResource {
	...
	let hdr_convert_bind_group_layout = interface.device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
	...
	let hdr_convert_bind_group = interface.device.create_bind_group(&wgpu::BindGroupDescriptor {
    label: None,
    ...
    entries: &[
        ...
        wgpu::BindGroupEntry { binding: 2, resource: wgpu::BindingResource::TextureView(&cube_storage_view) },
    ],
});

	let hdr_convert_pipeline_layout = interface.device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
		label: Some("HDR Convert Pipeline Layout"),
		bind_group_layouts: &[&hdr_convert_bind_group_layout],
		push_constant_ranges: &[],
	});

	let hdr_convert_pipeline = interface.device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
	...
	let sky_pipeline = interface.device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
	...
	let sky_bind_group = interface.device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                ...
                wgpu::BindGroupEntry { binding: 1, resource: wgpu::BindingResource::TextureView(&interface.depth_texture.create_view(&wgpu::TextureViewDescriptor::default())) },
                wgpu::BindGroupEntry { binding: 2, resource: wgpu::BindingResource::TextureView(&interface.intermediate_texture.create_view(&wgpu::TextureViewDescriptor::default())) },
				wgpu::BindGroupEntry { binding: 3, resource: wgpu::BindingResource::TextureView(&cube_sample_view) },
